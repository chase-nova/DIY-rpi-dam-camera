...
<img src="images/11-device-page.png" alt="기기 페이지: 왼쪽에 최신 프레임과 파일 경로, 실시간 화면 링크, 오른쪽에 카메라가 보고한 상태(파이 모델, 카메라, 간격, 온도, 전원, 업로드 수)" width="600">

*카메라 페이지 윗부분: **최신 프레임**(여기서는 흐리게 처리)과 §8에서 쓰는 실시간 화면 링크, 그리고 카메라가 마지막으로 보고한 상태. 이 예시 카메라는 문제가 있는 상태입니다 — 95.7 °C(일시 정지)와 저전압 — §3과 §11의 열·전원 조언이 막아 주는 바로 그 상황입니다.*
...
<img src="images/12-focus-view.jpg" alt="카메라 실시간 화면: 위쪽에 촬영지 이름, 시각, 프레임 번호, 아래에 먼 스카이라인이 선명한 카메라 이미지" width="600">

*`http://<pi-ip>:8080/`의 실시간 화면: 제목 줄에 촬영지, 시각, 프레임 번호가 보입니다. 이 스카이라인처럼 먼 곳의 디테일이 가장 선명해질 때까지 렌즈를 돌리세요. (가까운 건물은 개인정보 보호를 위해 흐리게 처리했습니다.)*

<img src="images/mounted-outdoors.jpg" alt="완전히 포장된 카메라 두 대: 창이 달린 밀봉 상자를 야외 난간에 설치한 모습" width="600">

*완전히 포장된 모습: 카메라마다 창 뒤에 넣어 상자를 밀봉했고, 설치할 준비가 되었습니다 (§9).*
...
- **내 영상**: **Manage → Locations**에서 내 촬영지를 엽니다. 매일의
  영상은 하루가 끝나고 약 30분 안에 저절로 나타납니다. 영상은 처음에
  **private**(비공개)이며, *private* / *public* 표시를 누르면 공개 사이트에
  보이거나 다시 숨길 수 있습니다. 마음에 들지 않는 날은 *Unassign*으로

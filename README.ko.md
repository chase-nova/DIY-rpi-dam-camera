...
10. [문제 해결](#10-문제-해결)
11. [개인정보, 안전, 라이선스](#11-개인정보-안전-라이선스)
...
```bash
ssh <user>@dam-cam-1.local        # 또는 ssh <user>@<pi-ip>
...
- **Name**: 영문자, 숫자, `-` 또는 `_` (예: `River-View`). 영상 파일
  이름의 일부가 되며, 카메라가 배정된 동안에는 바꿀 수 없습니다.
- **Time zone**: 그 장소의 시간대.
...
- **Video window** — 하루 종일, 정해진 시간, 또는 프리셋 *Daylight video
  (dawn → dusk)*(새벽 → 해 질 녘) / *Night video (dusk → dawn)*(해 질 녘 →
  새벽) (좌표 필요).
...
*[사진 15 — images/15-first-video.png: 첫 일일 영상]*
